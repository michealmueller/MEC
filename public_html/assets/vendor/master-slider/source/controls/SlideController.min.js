...
/* @endif */
;this.view.create(startSlide);if(this.so.preload===0){this.view.slideList[0].loadImages()}this.scroller=this.view.controller;if(this.so.wheel){var that=this;var last_time=new Date().getTime();this.wheellistener=function(event){var e=window.event||event.orginalEvent||event;e.preventDefault();var current_time=new Date().getTime();if(current_time-last_time<400){return}last_time=current_time;var delta=Math.abs(e.detail||e.wheelDelta);if($.browser.mozilla){delta*=100}var scrollThreshold=15;if(e.detail<0||e.wheelDelta>0){if(delta>=scrollThreshold){that.previous(true)}}else{if(delta>=scrollThreshold){that.next(true)}}return false};if($.browser.mozilla){this.slider.$element[0].addEventListener("DOMMouseScroll",this.wheellistener)}else{this.slider.$element.bind("mousewheel",this.wheellistener)}}if(this.slider.$element[0].clientWidth===0){this.slider.init_safemode=true}this.__resize();
/* @ifdef PRO */
;var that=this;if(this.so.deepLink){$(window).on("hashchange",function(){that.__onHashChanged()})}
/* @endif */
};p.index=function(){return this.view.index};p.count=function(){return this.view.slidesCount};p.next=function(checkLoop){this.skipTimer();this.view.next(checkLoop)};p.previous=function(checkLoop){this.skipTimer();this.view.previous(checkLoop)};p.gotoSlide=function(index){index=Math.min(index,this.count()-1);this.skipTimer();this.view.gotoSlide(index)};p.destroy=function(reset){this.dispatchEvent(new MSSliderEvent(MSSliderEvent.DESTROY));this.slider.destroy(reset)};p._destroy=function(){this._timer.reset();this._timer=null;$(window).unbind("resize",this.resize_listener);this.view.destroy();this.view=null;if(this.so.wheel){if($.browser.mozilla){this.slider.$element[0].removeEventListener("DOMMouseScroll",this.wheellistener)}else{this.slider.$element.unbind("mousewheel",this.wheellistener)}this.wheellistener=null}this.so=null};p.runAction=function(action){var actionParams=[];if(action.indexOf("(")!==-1){var temp=action.slice(0,action.indexOf("("));actionParams=action.slice(action.indexOf("(")+1,-1).replace(/\"|\'|\s/g,"").split(",");action=temp}if(action in this){this[action].apply(this,actionParams)}else{if(console){console.log('Master Slider Error: Action "'+action+'" not found.')}}};p.update=function(hard){if(this.slider.init_safemode&&hard){this.slider.init_safemode=false}this.__resize(hard);if(hard){this.dispatchEvent(new MSSliderEvent(MSSliderEvent.HARD_UPDATE))}};p.locate=function(){this.__resize()};p.resume=function(){if(!this.paused){return}this.paused=false;this._startTimer()};p.pause=function(){if(this.paused){return}this.paused=true;this._stopTimer()};p.currentTime=function(){return this._delayProgress};
/* @ifdef PRO */
;p.showLayer=function(layerId,delay){var layer=this.__findLayerById(layerId);if(layer){if(!delay){layer.start()}else{clearTimeout(layer.actionTimeout);layer.actionTimeout=setTimeout(this.showLayer,delay,layerId,0)}}};p.hideLayer=function(layerId,delay){var layer=this.__findLayerById(layerId);if(layer){if(!delay){layer.hide()}else{clearTimeout(layer.actionTimeout);layer.actionTimeout=setTimeout(this.hideLayer,delay,layerId,0)}}};p.toggleLayer=function(layerId,delay){var layer=this.__findLayerById(layerId);if(layer){if(!delay){layer.isShowing?layer.hide():layer.start()}else{clearTimeout(layer.actionTimeout);layer.actionTimeout=setTimeout(this.toggleLayer,delay,layerId,0)}}};p.showLayers=function(layerIds,delay){var self=this;$.each(layerIds.replace(/\s+/g,"").split("|"),function(index,layerId){self.showLayer(layerId,delay)})};p.hideLayers=function(layerIds,delay){var self=this;$.each(layerIds.replace(/\s+/g,"").split("|"),function(index,layerId){self.hideLayer(layerId,delay)})};p.toggleLayers=function(layerIds,delay){var self=this;$.each(layerIds.replace(/\s+/g,"").split("|"),function(index,layerId){self.toggleLayer(layerId,delay)})};
/* @endif */
;averta.EventDispatcher.extend(p)})(jQuery);